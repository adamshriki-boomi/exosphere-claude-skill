---
id: "components-input--overview"
title: "Components/Input"
name: "Overview"
source_url: "https://exosphere.boomi.com/iframe.html?id=components-input--overview&viewMode=docs"
scraped_at: "2026-04-21T08:43:31.642Z"
...
Specifies the input type (e.g., text, email, password).
...
text
 ...
 | `one of: text | currency | phone | password | number | email | tel | search | date | datetime-local | hidden | month | time | url | week` |
| clearable | 
...
Adds a button to toggle visibility of the password in password input fields.
...
Hides the increment and decrement spin buttons that are typically available for number inputs, allowing for a cleaner input field.
...
Disables the input, making it non-interactable.
...
Prevents the user from editing the input value.
...
Indicates whether the input is in an invalid state, typically used to trigger error styles and messages when the input doesn't meet the validation criteria.
...
Used for form validations in order to make it a required field to fill the input so it cannot be left blank.
...
Enables browser-based spell checking for the input, helping users catch typos as they type.
...
Shows an icon indicating input status (e.g., success or error).
...
Sets the type of footer message shown below the input (e.g., info or error).
...
Additional info text displayed when the user hovers over the info icon next to the input label.
...
Sets the size of the input (e.g., small, medium, large).
...
Sets the color or theme of the input.
...
Adds an icon before the input field.
...
Help or hint text displayed below the input.
...
Text on the tooltip shown when hovering over the clear button.
...
Controls whether special characters in the input should be automatically sanitized. Disabling this may allow richer input but could pose security risks if not handled properly.
...
Lets the browser suggest and automatically fill in values based on the user’s past input, like names or email addresses.
...
Hides the character count below the input. The character count only appears when 'maxLength' is set.
...
Function that is triggered when the input loses focus.
...
Function that is triggered when the input receives focus.
...
Function that is triggered when the input value is changed and the element loses focus.
...
Function that is triggered when the clear button is clicked.
...
Function that is triggered as the user types into the input.
...
Name attribute used when submitting the form.
...
Defines the format the input value must follow. Use a regular expression to enforce patterns like zip codes, phone numbers, or custom formats.
...
Provides a hint to browsers about which keyboard layout to show, making data entry easier for users (e.g., numeric keypad for numbers).

\-

 | \- | `one of: none | text | decimal | numeric | tel | search | email | url` |
| leadingIconLabel | 

Tooltip label for the leading icon.
...
Tooltip label for the status icon.
...
### Number Input With Min Max Attribute

### Number Input With No Spin Button
